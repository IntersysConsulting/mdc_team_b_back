...
any_ns = Namespace("carts", description="Endpoints that allow users to interact with their carts based on their session token.")

# Customer Section

get_cart_parser = any_ns.parser()
get_cart_parser.add_argument('Authorization', help="The autorization token bearer. This is a placeholder and should be handled with JWT.",
...
add_remove_item_parser.add_argument('Authorization', help="The autorization token bearer. This is a placeholder and should be handled with JWT.",
                             required=True, location="headers")
add_remove_item_parser.add_argument('product_id', help="The ObjectID of the product to be affected.", required=True )

update_item_parser = any_ns.parser()
update_item_parser.add_argument('Authorization', help="The autorization token bearer. This is a placeholder and should be handled with JWT.",
                             required=True, location="headers")
update_item_parser.add_argument('product_id', help="The ObjectID of the product to be affected.", required=True )
update_item_parser.add_argument('quantity', type=int, help="The new quantity of the product. Must be greater than 0.", required=True)
...
class UCart(Resource):
    ...
    def post(self):
        ...
        return jsonify({"statusCode":200, "data":{"Auth":token, "product_id":pid}})
        
    @any_ns.response(406, "The new quantity for the product is less than 1. Did you mean to use DELETE?")
    ...
    def put(self):
        ...
        return jsonify({"statusCode":code, "data":{"Auth":token, "product_id":pid, "quantity":quantity}})
